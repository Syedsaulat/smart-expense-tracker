...
- Went through every endpoint and made sure it matched what the assignment asked for (add, view, filter, totals, delete). The AI's first pass was close but I had to fix the category filtering to be case-insensitive — I changed the comparison in `view_expenses` to use `.lower()` on both sides.
- Wrote the `field_validator` for title and category from scratch so that whitespace-only strings like `"   "` get rejected and valid strings get trimmed. The AI didn't include this, but I thought it was an obvious edge case.
- Wrote two extra test cases myself — `test_rejects_whitespace_only_title` and `test_trims_title_and_category` — to cover the validator I added.
